...
If -UseGraph is specified, the site is created through the Microsoft Graph site creation API instead of through SharePoint CSOM. Submitting the creation request only requires the Microsoft Graph `Sites.Create.All` permission (delegated or application) - no SharePoint API permission is needed for that part. `Connect-PnPOnline` still requires a `-Url` to establish the connection itself, but the app registration does not need a SharePoint API permission just to create the site. If `-Wait` is also specified, checking the provisioning status does require a SharePoint-audience token (Microsoft's beta site-creation API currently exposes its status endpoint on the tenant's own `https://{tenant}.sharepoint.com` rather than `graph.microsoft.com`), so the app registration needs at least one SharePoint API permission for that combination to succeed. `-UseGraph` is only supported with `-Type CommunicationSite` or `-Type TeamSiteWithoutMicrosoft365Group` (the Graph endpoint has no template for a Microsoft 365 group-connected team site), and does not support `-Classification`, `-SiteDesign`, `-SiteDesignId`, `-PreferredDataLocation`, `-SensitivityLabel`, `-HubSiteId` or `-TimeZone`.
...
### -Wait
If specified the cmdlet will wait until the site has been fully created and all site artifacts have been provisioned by SharePoint. Notice that this can take a while. When used together with `-UseGraph`, checking the provisioning status requires a SharePoint-audience token for the tenant, so the connection's app registration needs at least one SharePoint API permission in addition to the Graph `Sites.Create.All` permission; without `-Wait`, `-UseGraph` only needs `Sites.Create.All`.
