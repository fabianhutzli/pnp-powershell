...
If -UseGraph is specified, the site is created through the Microsoft Graph site creation API instead of through SharePoint CSOM. Submitting the creation request only requires the Microsoft Graph `Sites.Create.All` permission (delegated or application) - no SharePoint API permission is needed for that part. `Connect-PnPOnline` still requires a `-Url` to establish the connection itself, but the app registration does not need a SharePoint API permission just to create the site. If `-Wait` is also specified, checking the provisioning status does require a SharePoint-audience token, because Microsoft's beta site-creation API currently exposes its status endpoint on the tenant's own `https://{tenant}.sharepoint.com` rather than `graph.microsoft.com`. A per-site `Sites.Selected` grant is **not** sufficient for this, since that grant can only be made against a site that already exists - a site being created for the first time cannot have been pre-authorized. To use `-Wait` reliably, the app registration needs a non-site-scoped SharePoint permission (e.g. Graph application permission `Sites.FullControl.All`), which defeats the minimal-permission goal `Sites.Create.All`-only usage is intended for; in that scenario, omit `-Wait` and treat a returned URL as "creation submitted", not "creation confirmed complete". `-UseGraph` is only supported with `-Type CommunicationSite` or `-Type TeamSiteWithoutMicrosoft365Group` (the Graph endpoint has no template for a Microsoft 365 group-connected team site), and does not support `-Classification`, `-SiteDesign`, `-SiteDesignId`, `-PreferredDataLocation`, `-SensitivityLabel`, `-HubSiteId` or `-TimeZone`.
...
### -Lcid
The language to use for the site. For more information, see Locale IDs supported by SharePoint at https://github.com/pnp/powershell/wiki/Supported-LCIDs-by-SharePoint. To get the list of supported languages on a SharePoint environment use: Get-PnPAvailableLanguage. When used together with `-UseGraph`, defaults to `en-US` if not specified.
...
### -Wait
If specified the cmdlet will wait until the site has been fully created and all site artifacts have been provisioned by SharePoint. Notice that this can take a while. When used together with `-UseGraph`, checking the provisioning status requires a SharePoint-audience token for the tenant; a non-site-scoped SharePoint permission is needed for this (a per-site `Sites.Selected` grant cannot work here, since it can only be made against a site that already exists). Without `-Wait`, `-UseGraph` only needs the Graph `Sites.Create.All` permission.
